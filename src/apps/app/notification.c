...
static void anim_label(app_t *app, lv_obj_t *obj, bool reverse) {
...
static void no_notification_create(notification_app_t *ht) {
    lv_obj_t * lv_app = lv_label_create(ht->screen);
    ht->lv_app = lv_app;
    lv_label_set_text_static(ht->lv_app, "\xEE\xA4\x80\nNo new\nnotifications");
    lv_obj_set_style_text_align(ht->lv_app, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(ht->lv_app, lv_color_hex(0x0fafa0), 0);
...
static int update(app_t *app) {
    ...
    if ( pinetimecosBLE.newNotification ) {
        ...
        pinetimecosBLE.newNotification = false;
        if (ht->empty) {
            lv_obj_del(ht->lv_app);
            screen_notification_create(ht);
            ht->empty = false;
        }
        show_not(app);
    }
