...
#define TASK_DELAY        5 /**< Task delay. Delays task for 200 ms */
TaskHandle_t  lcd_task_handle;

static void lcd_task_function (void * pvParameter)
{
    ...
    while (true)
    {
        ...
        vTaskDelay(TASK_DELAY);
        st7789_fill(0, 0, 240, 240, RGB2COLOR(255, 0, 0));
...
int main(void)
{
    ...
    /* Create task with priority set to 2 */
    UNUSED_VARIABLE(xTaskCreate(lcd_task_function, "LCD", configMINIMAL_STACK_SIZE + 200, NULL, 2, &lcd_task_handle));
